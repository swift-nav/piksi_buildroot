...
void callback_sbp_utc_time(u16 sender_id, u8 len, u8 msg[], void *context) {
  ...
  auto *time = reinterpret_cast<msg_utc_time_t*>(msg);

  tN2kMsg n2kMsg;
  u16 days_since_1970;
  double seconds_since_midnight;
  if (converter.Sbp259ToPgn126992(time, &n2kMsg, &days_since_1970,
  ...
  converter.ResetPgn129029CacheIfOld(time->tow);
  converter.SetPgn129029CacheFields(time->tow, days_since_1970,
                                    seconds_since_midnight);
  if (converter.IsPgn129029Ready(/*is_valid=*/true)) {
    n2kMsg.Clear();
...
void callback_sbp_pos_llh(u16 sender_id, u8 len, u8 msg[], void *context) {
  ...
  bool is_valid = (sbp_pos->flags & TIME_SOURCE_MASK) != NO_TIME;
...
void callback_sbp_dops(u16 sender_id, u8 len, u8 msg[], void *context) {
  ...
  bool is_valid = (sbp_dops->flags & TIME_SOURCE_MASK) != NO_TIME;
